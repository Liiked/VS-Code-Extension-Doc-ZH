...
用户或插件提供的自定义样式表会依赖 DOM 结构和类名。由于我们将它们视为内部实现，因此不会对其进行文档化。为了持续演进、重构或改进 VS Code，我们需要能够自由修改用户界面。对 DOM 的任何改动都可能破坏现有的自定义样式表，导致样式表提供者感到困扰，并因样式表失效造成 UI 显示异常，影响用户体验。

作为替代方案，VS Code 致力于提供设计完善、支持 UI 自定义的扩展 API。这些 API 配有文档、工具和示例，并会在后续所有 VS Code 版本中保持稳定。